...
"""
#################### 
# Fourier Neural Operator tester for square domains - for Fourth Year Project
...
def plot_prediction_comparison(model, dataloader, num_samples=3):
    ...
    with torch.no_grad():
        for i, (x, y) in enumerate(dataloader):
            ...
            im1 = axes[i, 0].imshow(input_grid[2, :, :], cmap='viridis')  
...
def main():
    ...
    test_csv_path = "test_63_square_200.csv"
    model_path = "data_32_square_1000_Test_2_good.pth"
    
    # Load test data
    test_dataloader = load_test_data(test_csv_path)
    
    # Initialize and load model
    model = FNO2D(in_channels=4, out_channels=1, modes1=12, modes2=12, width=256).to(device)
