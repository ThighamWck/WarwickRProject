"""
#################### 
# Forward Committor Solver - for Fourth Year Project
...
def solve_forward_committor_2D(phi, psi, bx, by, N):
    """
    Solve the 2D steady advection-diffusion equation with coordinate transform (x,y) -> (f(x,y), y):
        d_xx(u) + d_yy(u) + bx(x,y) * d_x(u) + by(x,y) * (d_y(u) + d_x(u)*f_y(x,y)) + d_yy(f)*d_x(u)
        + d_y(f)^2 * d_xx(u) + 2 * d_y(f) * d_yy(u) = 0
    on (0,1)^2 with Dirichlet BCs in x (u(0,y)=0, u(1,y)=1)
    ...
    """ We will use a sparse matrix form. The csr_matrix constructor takes the (rows, cols, data) lists and automatically sums duplicate entries for the same (row, column) pair.
        ...
        We range i from 1 to N+1 as we dont need to access the 0 and N+2 points as value is set (BCs). 
    """
    for i in range(1, N+1):
        ...
        for j in range(N+2):
            ...
            if i < N:
                rows.append(idx); cols.append(idx+(N+2)); data.append((1+f_y**2)/dx**2)
            if i ==N:
                #Including affect right boundary condition from diffusion in B
            ...
            if i > 1 and i < (N): #away from dirichlet boundaries
                ...
                if combined_x_advection >=0: # Flow is positive or zero - use backward difference
                    # We force u_(N+1,j) =0 so include 0 into solution B. Already a zero there so we are good.
                    rows.append(idx); cols.append(idx); data.append(combined_x_advection/dx)
                else:  # Flow is negative - use forward difference
                    rows.append(idx); cols.append(idx); data.append(-combined_x_advection/dx)
                    rows.append(idx); cols.append(idx+(N+2)); data.append(combined_x_advection/dx)
                ...
                if combined_x_advection >=0: # Flow is positive or zero - use backward difference
                    ...
                    rows.append(idx); cols.append(idx); data.append(-combined_x_advection/dx)
                    # We force u_(N+1,j) =1 so include -combined_x_advection/dx into solution B
            ...
            if i < N:
                ...
                rows.append(idx); cols.append((i)*(N+2) + (j+1)%(N+1)); data.append((2*f_y)/(4*dx*dy))
            #At right boundary, conditions lead to mixed derivatives cancelling for i ==N
    ...
    u = U.reshape((N, N+2))
    u = anp.vstack((anp.zeros(N+2), u, anp.ones(N+2)))
